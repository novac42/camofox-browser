...
async function startServer(
  ...
  if (pluginCfg?.interactiveLogin != null) env.CAMOFOX_INTERACTIVE_LOGIN = String(pluginCfg.interactiveLogin);
  if (pluginCfg?.interactiveLoginTtlSec != null) env.CAMOFOX_INTERACTIVE_LOGIN_TTL_SEC = String(pluginCfg.interactiveLoginTtlSec);
  if (pluginCfg?.maxLoginSessions != null) env.MAX_LOGIN_SESSIONS = String(pluginCfg.maxLoginSessions);
...
export default function register(api: PluginApi) {
  ...
  api.registerTool((ctx: ToolContext) => ({
    name: "camofox_start_login",
    description:
      "Start an interactive login session for the current user. Returns a temporary login URL that a human can open to complete authentication.",
    parameters: {
      type: "object",
      properties: {
        url: { type: "string", description: "Login URL to open (e.g. https://example.com/login)" },
        allowedDomain: { type: "string", description: "Optional domain scope for login navigation (defaults to URL hostname)" },
        ttlSec: { type: "number", description: "Optional session TTL in seconds (default 600)" },
      },
      required: ["url"],
    },
    async execute(_id, params) {
      const userId = ctx.agentId || fallbackUserId;
      const sessionKey = ctx.sessionKey || "default";
      const { url, allowedDomain, ttlSec } = params as {
        url: string;
      ...
      if (!envCfg.apiKey) {
        throw new Error(
          "CAMOFOX_API_KEY is required for interactive login. Set it for both server and plugin environments."
        );
      }

      const result = await fetchApi(baseUrl, `/sessions/${encodeURIComponent(userId)}/login/start`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${envCfg.apiKey}`,
        },
        body: JSON.stringify({ sessionKey, url, allowedDomain, ttlSec }),
      });
  ...
  api.registerTool((ctx: ToolContext) => ({
    name: "camofox_check_login",
    description:
      "Check status of an interactive login session. Use this to poll until status changes from pending.",
    parameters: {
      type: "object",
      properties: {
        loginId: { type: "string", description: "Interactive login session id from camofox_start_login" },
      },
      required: ["loginId"],
    },
    async execute(_id, params) {
      ...
      const result = await fetchApi(
        baseUrl,
        `/sessions/${encodeURIComponent(userId)}/login/${encodeURIComponent(loginId)}/status`,
        {
          headers: {
            Authorization: `Bearer ${envCfg.apiKey}`,
          },
  ...
  api.registerTool((ctx: ToolContext) => ({
    name: "camofox_cancel_login",
    description:
      "Cancel an interactive login session if the user is done or login is no longer needed.",
    parameters: {
      type: "object",
      properties: {
        loginId: { type: "string", description: "Interactive login session id from camofox_start_login" },
    ...
    async execute(_id, params) {
      ...
      const result = await fetchApi(
        baseUrl,
        `/sessions/${encodeURIComponent(userId)}/login/${encodeURIComponent(loginId)}/cancel`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${envCfg.apiKey}`,
