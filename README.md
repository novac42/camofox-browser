...
- **Interactive Login** - generate a temporary login URL and let a human complete auth on demand
...
**Tools:** `camofox_create_tab` · `camofox_snapshot` · `camofox_click` · `camofox_type` · `camofox_navigate` · `camofox_scroll` · `camofox_screenshot` · `camofox_close_tab` · `camofox_list_tabs` · `camofox_start_login` · `camofox_check_login` · `camofox_cancel_login` · `camofox_import_cookies`
...
### On-Demand Interactive Login (Beta)

Use this flow when cookie import is inconvenient or unsafe:

1. Enable feature flag:

```bash
export CAMOFOX_API_KEY="your-generated-key"
export CAMOFOX_INTERACTIVE_LOGIN=true
```

2. Start login from the agent (`camofox_start_login`) with a site login URL.
3. Open returned `loginUrl` in a normal browser and finish authentication.
4. Poll with `camofox_check_login` until status becomes `completed`.

Server API (agent-facing):
...
| `POST` | `/sessions/:userId/login/start` | Start interactive login session |
| `GET` | `/sessions/:userId/login/:loginId/status` | Get interactive login status |
| `POST` | `/sessions/:userId/login/:loginId/complete` | Mark interactive login complete |
| `POST` | `/sessions/:userId/login/:loginId/cancel` | Cancel interactive login session |
...
| `CAMOFOX_INTERACTIVE_LOGIN` | Enable interactive login endpoints/UI | `false` |
| `CAMOFOX_INTERACTIVE_LOGIN_TTL_SEC` | Default interactive login TTL (seconds) | `600` |
| `CAMOFOX_ADMIN_KEY` | Required for `POST /stop` | - |
| `CAMOFOX_COOKIES_DIR` | Directory for cookie files | `~/.camofox/cookies` |
| `MAX_SESSIONS` | Max concurrent browser sessions | `50` |
| `MAX_LOGIN_SESSIONS` | Max concurrent pending interactive logins | `100` |
